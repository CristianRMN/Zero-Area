...
func _process(delta):
	...
	if position.y > initial_position.y + move_distance and position.x > initial_position.x + move_distance:
		direction = -1
	elif position.y < initial_position.y - move_distance and position.x < initial_position.x - move_distance:
		direction = 1

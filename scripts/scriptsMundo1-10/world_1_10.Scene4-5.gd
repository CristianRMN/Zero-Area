...
@onready var bananaK = $platano_recupera_vida/BananaKeyboardX
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("alimentacion"):
		if is_instance_valid(bananaK) and bananaK.visible:
			life.heal(10)
			bananaK.visible = false  
		elif is_instance_valid(bananaX) and bananaX.visible:
			life.heal(10)
			bananaX.visible = false  
